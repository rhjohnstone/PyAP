...
samples_per_expt = 100
for i in xrange(num_gs):
    samples = np.zeros((nums_expts[-1], samples_per_expt))
    ...
    for n in xrange(nums_expts[-1]):
        ...
        sl_sample_idx = npr.randint(0, len(temp_chain), samples_per_expt)
        samples[n, :] = temp_chain[sl_sample_idx]
    print variances
    for a, N_e in enumerate(nums_expts):
        colour = plt.cm.winter(color_idx[a])
        # MLE fit
        loc, scale = norm.fit(samples[:N_e,:].flatten())
    ...
    fig.tight_layout()
    plt.show()
    fig.savefig(h_png_dir+'sl_mle_pred_and_h_post_pred_multiple_samples_per_sl_{}.png'.format(g_parameters[i]))
    plt.close()
