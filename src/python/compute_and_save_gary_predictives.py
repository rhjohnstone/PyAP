...
for i in xrange(num_gs):
    if pyap_options["model_number"]==4:
        ...
        if i==0:
            xs[i, :] = np.linspace(mins[i]+5, maxs[i]+1, num_pts)
        elif i==1:
            xs[i, :] = np.linspace(mins[i]-1, maxs[i]+0.5, num_pts)
        elif i==2:
            xs[i, :] = np.linspace(mins[i]-0.5, maxs[i]+0.5, num_pts)
        elif i==3:
            xs[i, :] = np.linspace(mins[i]-3, maxs[i]+6, num_pts)
        elif i==4:
            xs[i, :] = np.linspace(mins[i]+7, maxs[i]+1, num_pts)
        elif i==5:
            xs[i, :] = np.linspace(mins[i]-5, maxs[i]+9, num_pts)
        elif i==6:
            xs[i, :] = np.linspace(mins[i]-1, maxs[i]+0.5, num_pts)
        elif i==8:
