...
def do_mcmc(trace_number, ap_model, expt_trace, temperature):#, theta0):
    ...
    start = time.time()
    cmaes_dir, cmaes_path = ps.roche_cmaes_path(model_number, trace_number)
    try:
...
def do_everything(trace_number):
    expt_trace_path = ps.roche_trace_path(trace_number)
    try:
        expt_times, expt_trace = np.loadtxt(expt_trace_path, delimiter=',').T
    except:
    ...
    temperature = 1
    chain_dir, chain_file = ps.roche_data_clamp_exp_scaled_mcmc_file(model_number, trace_number)
...
num_cores = min(how_many_traces, 16)  # 16 for arcus-b
