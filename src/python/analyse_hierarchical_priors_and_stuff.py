...
for i in xrange(num_gs):
    ...
    ax1.plot(x, invgamma.pdf(x, updated_eta[2], scale=updated_eta[3]), lw=2, label="first new eta")
    ax1.legend()
    old_mode = beta/(alpha+1.)
    new_mode = updated_eta[3] / (updated_eta[2]+1.)
    x = np.linspace(0, 4*original_gs[i], num_prior_pts)
    old_normalising_factor = 1.-norm.cdf(0, loc=old_eta_js[i,0], scale=np.sqrt(old_mode/old_eta_js[i,1]))
    new_normalising_factor = 1.-norm.cdf(0, loc=updated_eta[0], scale=np.sqrt(new_mode/updated_eta[1]))
    ax2.plot(x, norm.pdf(x, loc=old_eta_js[i,0], scale=np.sqrt(old_mode/old_eta_js[i,1]))/old_normalising_factor, lw=2, label="old")
    ax2.plot(x, norm.pdf(x, loc=updated_eta[0], scale=np.sqrt(new_mode/updated_eta[1]))/new_normalising_factor, lw=2, label="new")
